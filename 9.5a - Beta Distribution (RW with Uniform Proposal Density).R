...
# PROGRAM   9.5a - Beta Distribution (RW with Uniform Proposal Density)
...
#           = 0               if  x \notin [0,1]
#
# To achieve this we use the proposal density defined by:
...
# Metropolis-Hastings algorithm. Parameter definitions
    ...
    # proposal_width = width of uniform proposal density defined by 
    # y_{t+1} = X^(t) + \epsilon_t  where \epsilon_t ~ U[-d/2, d/2]


# 1.1 - Defining the function / macro 
betaMH <- function(n, a, b, start, proposal_width){
...
set.seed(12345)
MH_Output <- betaMH(burnin + mcmc, 2, 4, start = 0.5, proposal_width = 1)
